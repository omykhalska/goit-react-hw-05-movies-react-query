...
export async function fetchPopularMovies() {
  const url = `${BASE_URL}/trending/movie/day?api_key=${API_KEY}&language=ru-RU`;
...
export async function fetchMovieReviews(movieId) {
  const url = `${BASE_URL}/movie/${movieId}/reviews?api_key=${API_KEY}&language=ru-RU`;
  const { data } = await axios(url);
  return data;
}

export async function fetchMoviesBySearch(query, page) {
  const url = `${BASE_URL}/search/movie?api_key=${API_KEY}&query=${query}&page=${page}&language=ru-RU`;
  const { data } = await axios(url);
  return data;
}
